...
{
	public static bool IsMegaCrypter(string FileID)
	{
		if (string.IsNullOrEmpty(FileID))
		{
			return false;
		}
		return FileID.ToLower().StartsWith("megacrypter.com".ToLower()) & (FileID.Split('$').Length == 3);
	}
	...
	{
		if (!IsMegaCrypter(FileID))
		{
			throw new ApplicationException("Error, not MegaCrypter link");
		}
		...
		foreach (string item in list)
		{
			...
			Conexion.Respuesta respuesta = Conexion.SendJSON("http://megacrypter.com/api", jSON, "", SendAppId: false);
			if (respuesta.Excepcion == null)
			{
				...
				try
				{
					...
					{
						throw new ApplicationException("Invalid Megacrypter link (" + text + ")");
					}
					dictionary = (Dictionary<string, object>)JsonConvert.DeserializeObject(text, typeof(Dictionary<string, object>));
					if (dictionary.ContainsKey("error"))
					{
						throw new ApplicationException("Invalid Megacrypter link (" + Conversions.ToString(dictionary["error"]) + ")");
					}
				}
				catch (Exception ex)
				{
					...
					Exception ex2 = ex;
					Log.WriteError("Error getting the info in MegaCrypter - Error: " + ex2.ToString());
					informacionFichero.Err = Conexion.TipoError.Otros;
					informacionFichero.Errtxt = "Error getting the info in MegaCrypter: " + ex2.Message;
			...
			if (respuesta.Excepcion is WebException)
			{
				...
				Log.WriteError("Error getting the info in MegaCrypter: " + respuesta.Excepcion.Message + " - Message received: " + respuesta.Mensaje);
				result = informacionFichero;
			}
			else
			{
